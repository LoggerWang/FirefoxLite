...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public ParcelFileDescriptor openTun() {
        ...
        try {
            //添加白名单
            //添加允许访问 VPN 连接的应用程序。如果此方法至少被调用一次，则仅允许通过此方法添加的应用程序（而不是其他应用程序）访问。
            // 否则（如果从未调用此方法），默认情况下允许所有应用程序。如果添加了一些应用程序，其他未添加的应用程序将使用网络，就好像 VPN 没有运行一样。
            // VpnService.Builder 可能只有一组允许的应用程序或一组不允许的应用程序，但不能同时具有两者。
            // 在调用 addDisallowedApplication 之后调用此方法，反之亦然，将抛出 UnsupportedOperationException。
            // packageName 必须是当前安装的应用程序的规范名称。如果没有这样的应用程序，则抛出 PackageManager.NameNotFoundException。
            Log.d("legend", "=======vpn添加白名单addAllowedApplication==========getPackageName()==" + getPackageName());
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        ...
        if (mDisplayBytecount) {
            ...
            //MultiDataHelper.getInstance().setValueText(netstat);
            ...
            Log.e("muccc", "in_" + in + "_out_" + out + "_diffIn_" + diffIn + "_diffOut_"
                    + diffOut + "_duration_" + duration + "_lastPacketReceive_" + lastPacketReceive + "_byteIn_" + byteIn + "_byteOut_" + byteOut);
            sendMessage(in, out, diffIn, diffOut, duration, String.valueOf(lastPacketReceive), byteIn, byteOut);
        }
    }
    ...
    //sending message to main activity
    private void sendMessage(long in, long out, long diffIn, long diffOut, String duration, String lastPacketReceive, String byteIn, String byteOut) {
        Intent intent = new Intent("connectionState");
